  ...
  async load() {
    switch (this.identifier.type) {
      case 'remote':
        ...
        // Now get README from remote if exists
        const readmeUrl = urljoin(this.path, 'README.md')
      ...
      default: // assume local
        ...
        const pathToReadme = path.join(this.path, 'README.md')
        if (fs.existsSync(pathToReadme)) {
          const readme = fs.readFileSync(pathToReadme).toString()
          this._readme = readme
        }
    }
