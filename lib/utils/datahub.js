...
export class DataHub extends EventEmitter {
  ...
  async push(pkg) {
    ...
    this._debugMsg('Getting rawstore auth')

    var rawstoreUploadCreds = await this.rawstoreAuthorize(resources)

    this._debugMsg('Uploading to rawstore')
    ...
    try {
      const res = await this._fetch('/source/upload', {
        method: 'POST',
        body: spec
      })
      if (res.status === 200) {
      ...
      console.dir(err)
      // TODO: use responseFromSpecStoreπ (?)
      throw Error(`Spec store upload failed: ${err}`)
    }
  }
