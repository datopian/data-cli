...
module.exports.login = async function() {
  ...
  try {
    ...
    const token = module.exports._extractToken(urlWithToken)
		...
		module.exports._saveUserInfo(userInfo, token)
...
// Save token, email and username of the user in ~/.datahub.json
module.exports._saveUserInfo = (userInfo, token) => {
	let info = {
		token: token,
	...
	// If username is not given then use hash of email
	if (!info.username) {
		info.username = md5(info.email)
	}
	const homeDir = process.env.HOME || process.env.USERPROFILE || process.env.HOMEPATH
	fs.writeFileSync(homeDir + '/.datahub.json', JSON.stringify(info))
}
