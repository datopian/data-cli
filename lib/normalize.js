...
const normalizeContributors = dp => {
  const normalizeContirutorsTitle = dp => {
    for (const idx in dp.contributors) {
      if (dp.contributors[idx].title) {
        dp.contributors[idx].title = dp.contributors[idx].title
      } else if (dp.contributors[idx].name) {
        dp.contributors[idx].title = dp.contributors[idx].name
        delete dp.contributors[idx].name
      }
    }
  }
  if (dp.author) {
