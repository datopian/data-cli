  ...
  async push(pkg) {
    ...
    let uploads = resources.map(async resource => {
      ...
      lodash.forEach(creds.upload_query, (v,k) => {
        // not sure why we have to remove these
        // if we did not get error like
        // Error uploading to rawstore for datapackage.json with code 403 reason
        // <Error><Code>AccessDenied</Code><Message>Invalid according to Policy: Extra input fields: content-md5 ...
        if (k != 'Content-Length' && k != 'Content-MD5') {
          formData.append(k,v)
        }
      })
