...
export class DataStream {
  constructor(path_) {
    this.descriptor = parsePath(path_)
  }

  /**
 * Create reabale stream
 * @param source. Can be:
 * path to local file
 * path to remote file
 * @returns Promise with readable stream object on resolve
 */
  getRawStream(descriptor=this.descriptor) {
    return new Promise((resolve, reject) => {
      if (descriptor.pathType === 'remote') {
        // create readable stream from remote file
        request.get(descriptor.path)
        ...
        resolve({ stream: fs.createReadStream(descriptor.path) })
      }
    })
  }
