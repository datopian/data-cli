  ...
  */
  ...
  _rows() {
    return (async () => {
      if (Object.keys(parserDatabase).indexOf(this.descriptor.format) !== -1) {
        ...
        throw Error(`We do not have a parser for that format: ${this.descriptor.format}`)
      }
    })()
  }
  ...
  get addSchema() {
    ...
    if (knownTabularFormats.indexOf(this.descriptor.format) === -1) {
      throw Error('Resource is not in known tabular format.')
    }
    return (async () => {
      const rows = await toArray(await this.rows)
      const headers = rows.shift()
      this.descriptor.schema = infer(headers, rows)
    })()
  }
}
...
export class ResourceInline extends Resource {
  constructor(descriptor, {basePath}={}) {
    ...
    if (lodash.isString(this.descriptor.data)) {
      this._buffer = new Buffer(this.descriptor.data)
    } else { // it is json/javascript
      this._buffer = new Buffer(JSON.stringify(this.descriptor.data))
    }
  }
...
export class Package {
  ...
  addResource(resource) {
    ...
    } else if (lodash.isObject(resoure)) { // it is already a resource object!
      this.descriptor.resources.push(resource.descriptor)
      this.resources.push(resource)
    } else {
      throw Error(`addResource requirese a resource descriptor or an instantiated resources but got: ${resource}`)
    }
  }
