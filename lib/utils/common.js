  ...
  get stream() {
    return new Promise((resolve, reject) => {
      if (this.descriptor.pathType === 'remote') {
        ...
        const protocol = this.descriptor.path.includes('https') ? https : http
        protocol.get(this.descriptor.path, res => {
          resolve({ stream: res })
        }, error => {
          reject(error)
        })
        ...
        resolve({ stream: fs.createReadStream(this.descriptor.path) })
      }
    })
  }
