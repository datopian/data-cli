...
class DataHub extends EventEmitter {
  ...
  async push(dataset, options) {
    ...
    // Upload to SpecStore
    const spec = makeSourceSpec(rawstoreUploadCreds, this._ownerid, this._owner, dataset.descriptor.name, options)
...
const makeSourceSpec = (rawstoreResponse, ownerid, owner, dataset, options) => {
